# Kalma X + Bluesky Test Runbook

Draft-only testing plan for the first two live social accounts.

## Current Scope

- Channels: X and Bluesky.
- Posting mode: manual publish only.
- CTA: read the first devlog and follow for updates.
- Do not use: Steam CTA, demo claim, release date, price, Discord URL, exact mechanics, comparables, or system requirements.
- Confirmed facts: Kalma, Perttula Game Studio, first-person survival horror, psychological horror positioning, PC, Unreal Engine 5.7, solo dev from Finland, dying northern town buried under snow.

## Account Setup Checklist

| Task | X | Bluesky | Notes |
| --- | --- | --- | --- |
| Handle chosen | [fill] | [fill] | Prefer studio handle if one account will cover future games. |
| Display name set | Perttula Game Studio | Perttula Game Studio | Or `Kalma` if the account is game-specific. |
| Bio set | [todo] | [todo] | Keep the same core wording across both. |
| Website link | https://perttulagamestudio.com/ | https://perttulagamestudio.com/ | Use devlog link in first posts. |
| Profile image | [todo] | [todo] | Studio logo is safer until Kalma logo exists. |
| Banner image | [todo] | [todo] | Use snowy key image or neutral studio banner. |
| 2FA enabled | [todo] | [todo] | Store recovery codes in password manager. |

## Bio Drafts

### Studio Account

Perttula Game Studio. One-person indie studio from Finland. Building Kalma, a first-person survival horror game set in a dying northern town.

### Game Account

Kalma is a first-person survival horror game from Perttula Game Studio. A desperate message. A dying northern town. Snow over everything.

## First Test Posts

Use one post per day at first. Do not post the same wording to both platforms at the exact same minute.

### X Test 1

Kalma is a first-person survival horror game set in a dying northern town buried under snow.

A desperate message. Empty streets. Something wrong beneath the silence.

Solo dev from Finland. Built with Unreal Engine 5.7.

First look: https://perttulagamestudio.com/devlog/kalma-first-look

### Bluesky Test 1

Working on Kalma, a first-person survival horror game set in a dying northern town.

Snow everywhere. Empty streets. A desperate message from someone you have not heard from in years.

Solo dev from Finland. Built with Unreal Engine 5.7.

First look: https://perttulagamestudio.com/devlog/kalma-first-look

### X Test 2

One thing I want Kalma to get right: the town should feel like a place you should have left alone.

Snow, empty streets, slow dread, and a message you probably should not have followed.

First-person survival horror.

Read the first devlog: https://perttulagamestudio.com/devlog/kalma-first-look

### Bluesky Test 2

Kalma dev note:

The town matters as much as the threat.

I want the snow, empty streets, and silence to do real work before the player understands what is wrong.

First-person survival horror, solo-built in Finland with Unreal Engine 5.7.

Read the first devlog: https://perttulagamestudio.com/devlog/kalma-first-look

## Testing Rhythm

| Day | Action | Goal |
| --- | --- | --- |
| Day 1 | Publish one intro post on X. | Baseline impressions and profile visits. |
| Day 2 | Publish one intro post on Bluesky. | Compare tone and early follows. |
| Day 3 | Reply/comment from the account for 15 minutes. | Make the account look alive without spam. |
| Day 4 | Publish one atmosphere post on X with screenshot. | Test image vs text-only. |
| Day 5 | Publish one atmosphere post on Bluesky with screenshot. | Test dev-community response. |
| Day 6 | Review metrics and keep the best-performing wording. | Learn before scaling. |
| Day 7 | Rest or repost only if there is real new context. | Avoid looking automated. |

## KPI Tracker

| Date | Channel | Post | Asset | Impressions | Likes | Reposts | Replies | Profile visits | Follows | Link clicks | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
|  | X | Test 1 | Text only |  |  |  |  |  |  |  |  |
|  | Bluesky | Test 1 | Text only |  |  |  |  |  |  |  |  |
|  | X | Test 2 | Screenshot |  |  |  |  |  |  |  |  |
|  | Bluesky | Test 2 | Screenshot |  |  |  |  |  |  |  |  |

## Manual Publish Safety

- Read the post once out loud before publishing.
- Check every link opens in a private browser window.
- Use the main website URL in account profiles, but use the devlog URL in individual posts when the copy points to a specific first look or dev note.
- Do not add hashtags if they push X over 280 characters.
- Do not mention Steam until the Steam page exists.
- Do not mention a demo, release date, or system requirements until confirmed.
- Screenshot posts should show actual Kalma visuals, not placeholder art.
